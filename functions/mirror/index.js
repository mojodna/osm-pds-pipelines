...
const FILES_TO_MIRROR =
  /((planet|history)-\d{6}.osm.pbf|changesets-\d{6}.osm.bz2)/;
    ...
    (err, results) => {
      ...
      return async.forEachLimit(
        ...
        (info, done) => {
          ...
          if (
            path.extname(info.filename) !== ".md5" &&
            localFiles.indexOf(`${year}/${info.filename}.md5`) < 0
          ) {
            return request.get(
              `${HTTP_SOURCE_PREFIX}${info.path}.md5`,
              (err, rsp, body) => {
                if (err) {
                  return done(err);
                }
                ...
                return async.parallel(
                  {
                    ...
                    submitJob: (done) => {
                      ...
                      if (
                        (extension.endsWith(".pbf") ||
                          extension.endsWith(".osm.bz2")) &&
                        latestByType[type] === Number(date)
                      ) {
                        jobs.push(
                          async.apply(
                            mirror,
