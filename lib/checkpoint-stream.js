...
/**
 * function that wraps a source stream from osm-replication-streams
 * and adds checkpointing on S3 
 * 
 * @param {*} source - source stream
 * @param {*} callback - (err, stream) callback - returns err or the stream
 */
const checkpointStream = (source, callback) => {
  loadStartingSequence((err, initialSequence) => {
    ...
    const stream = source({
      infinite: false,
      initialSequence: initialSequence != null ? initialSequence + 1: null,
      checkpoint
    })
